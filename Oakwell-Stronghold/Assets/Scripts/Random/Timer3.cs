...
public class Timer3 : MonoBehaviour
{
    ...
    public float countdown = 120.0f; // Every level will count down from 2 minutes. CHANGE IN EDITOR, IF YOU MUST.
    static bool active;
    ...
    void Update()
    {
        ...
        if (countdown >= 90) // player still has 3 stars til a minute's passed
        {
            DataManager.Instance.Level3Stars = 3;
        }
        ...
        if (countdown <= 89f)
        {
            ...
            if (!alreadyPlayedA) // only plays this sound effect / reduces score by one / saves score once.
            {
                ...
                alreadyPlayedA = true;
                DataManager.Instance.Level3Stars = 2; // damn, they down to gold
            }
        }
        ...
        if (countdown <= 59f)
        {
            ...
            if (!alreadyPlayedB) // only plays this sound effect once. removing this will cause audio spam.
            {
                starLost.PlayOneShot(starBloop);
                alreadyPlayedB = true;
                DataManager.Instance.Level3Stars = 1; // ZAMN, they down to SILVER
            }
        }
    }
}