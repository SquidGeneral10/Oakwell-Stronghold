...
public class Timer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(countdown >= 90) // player still has 3 stars til 30 seconds have passed
        {
        ...
        if(countdown <= 89f)
        {
            ...
            if (!alreadyPlayedA) // only plays this sound effect / reduces score by one / saves score once.
            {
                ...
                alreadyPlayedA = true;
                DataManager.Instance.Level1Stars = 2; // damn, they down to gold
            }
        }
        ...
        if(countdown <= 59f)
        {
            ...
            if (!alreadyPlayedB) // only plays this sound effect once. removing this will cause audio spam.
            {
                starLost.PlayOneShot(starBloop);
                alreadyPlayedB = true;
                DataManager.Instance.Level1Stars = 1; // ZAMN, they down to SILVER
            }
        }
    }
}